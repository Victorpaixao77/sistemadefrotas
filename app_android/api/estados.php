<?php
/**
 * API Estados - Lista estados (UF) para seleção em rotas - App Android
 * GET: retorna lista de estados (igual ao portal web motorista)
 */
...
try {
    $conn = getConnection();
    $stmt = $conn->query('SELECT id, uf, nome FROM estados ORDER BY nome');
    if ($stmt) {
        $estados = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (PDOException $e) {
    error_log('API estados: ' . $e->getMessage());
}

if (empty($estados)) {
    // Fallback: lista fixa de UFs do Brasil (se tabela estados não existir)
    $estados = [
