<?php
/**
 * 🧪 Teste SEFAZ - Cert + Key Separados
 * 📋 Testa usando certificado.pem e chave.pem separadamente
 */
...
<body>
    <div class='container'>
        <h1>🧪 Teste SEFAZ - Cert + Key Separados</h1>
...
try {
    ...
    if ($cert_existe && $key_existe) {
        ...
        echo "<p><strong>Permissões certificado.pem:</strong> <code>$perm_cert_oct</code></p>";
        echo "<p><strong>Permissões chave.pem:</strong> <code>$perm_key_oct</code></p>";
        echo "<p><strong>Legível pelo PHP:</strong> " . (is_readable($cert_pem) && is_readable($key_pem) ? '✅ Sim' : '❌ Não') . "</p>";
    }
    ...
    if ($cert_existe && $key_existe) {
        echo "<div class='card'>
            <h3>🧪 Testando SEFAZ com Cert + Key Separados</h3>";
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:nfe="http://www.portalfiscal.inf.br/nfe/wsdl/NfeStatusServico4">
    ...
    <soap:Body>
        ...
        // Configurações do certificado separado
